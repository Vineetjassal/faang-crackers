...
const COMPANY_COLORS = {
  Google:'#ea4335', Meta:'#0081fb', Amazon:'#ff9900',
  Apple:'#555555', Netflix:'#e50914', Microsoft:'#00a4ef', OpenAI:'#10a37f'
};
...
function renderProfiles(profiles) {
  ...
  if (!profiles.length) {
    grid.innerHTML = '<p style="text-align:center;color:#aaa;padding:40px;grid-column:1/-1;">No profiles found. Try different filters.</p>';
    return;
  }
  grid.innerHTML = profiles.map(p => `
    <div class="profile-card">
      <div class="company-badge" style="background:${COMPANY_COLORS[p.company]}22;color:${COMPANY_COLORS[p.company]}">${p.company}</div>
      <h3>${p.name}</h3>
      <div class="college">🎓 ${p.college}</div>
      <span class="role-badge">${ROLE_LABELS[p.role] || p.role}</span>
      ${p.tip ? `<div class="tip">"${p.tip}"</div>` : ''}
      <div class="meta">${p.yoe > 0 ? `${p.yoe} yr${p.yoe > 1 ? 's' : ''} exp &bull; ` : 'Fresh grad &bull; '}Offer: ${p.year}</div>
      ${p.linkedin ? `<a class="linkedin-link" href="${p.linkedin}" target="_blank" rel="noopener">View LinkedIn &rarr;</a>` : ''}
    </div>
  `).join('');
}
